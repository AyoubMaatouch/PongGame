...
export class UsersController {
    ...
    async AddUsersToRoomsbyId(@Body() user: AddedUsersDto, @Param('id') param: Number, @Req() req: Request) {
        if (user.room_password) {
            ...
            if (!status) throw new HttpException('Password Invalid', HttpStatus.UNAUTHORIZED);
        }
    ...
    @HttpCode(200)
    @UseInterceptors(FileInterceptor('avatar'))
    async UpdateRoom(@Param('room_id') room_id, @Body() RoomInfoDto: RoomInfoDto, @UploadedFile() file, @Req() req: Request) {
        // here you after succesfully creating room add the creator as the the owner
        const user_info = await this.UsersService.getUserbyLogin(req.user['userLogin']);
        const user = user_info.user_id;
        // const user = 1;
    ...
    @HttpCode(200)
    async GetAllBlockedUsers(@Req() req) {
        // add JWT ID user
        const user_info = await this.UsersService.getUserbyLogin(req.user['userLogin']);
        const user = user_info.user_id;
        return await this.UsersService.GetAllBlockedUsers(user).catch (
            ()=> {
                throw new HttpException('NOT FOUND', HttpStatus.NOT_FOUND);
            }
        );
    ...
    async getMe(@Req() req: Request) {
            try {
                ...
                const user_info = await this.UsersService.getUserbyLogin(req.user['userLogin']);
                const user = user_info.user_login;
                
                return await this.UsersService.getUserbyLogin(user).catch(
                   (err) => {
                    throw new HttpException('NOT FOUND', HttpStatus.NOT_FOUND);
                    }
                );
    ...
    async getMachHistory(@Req() req, @Param('user_id') user_id) {
        const user_info = await this.UsersService.getUserbyLogin(req.user['userLogin']);
        const user = user_info.user_id;
        const matches = await this.UsersService.GetMatchHistory(user_id); 
    ...
    async setData(@Param('login') login: any, @Req() req, @UploadedFile() file, @Body() userDataDto: userDataDto) {
        ...
        return await this.UsersService.updateUserData(Number(userRecord.user_id), userDataDto).catch((err) => {
    ...
    @Get('/msg/:room_id')
    @HttpCode(200)
    async getAllChats(@Req() req, @Param('room_id') room_id: number) {
        const user_info = await this.UsersService.getUserbyLogin(req.user['userLogin']);
        const user = user_info.user_id;
        return await this.UsersService.getAllChats(user, room_id).catch((error) => {
