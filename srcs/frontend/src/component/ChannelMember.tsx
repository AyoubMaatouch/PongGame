...
function ChannelMemeber({ id, name, avatar, isAdmin, onMute, onBlock, setMember, isFriend, isOwner, roomId, role }: Props) {
    const value = useColorModeValue('blackAlpha.200', 'whiteAlpha.200');
    const [hover, setHover] = useState<any>(false);
    const { dispatch, state, selectedChat, socket } = useContext<any>(ChatContext);
