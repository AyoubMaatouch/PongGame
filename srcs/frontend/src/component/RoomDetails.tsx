...
function RoomDetails() {
    ...
    useEffect(() => {
        axios.get(MEMBERS + selectedChat.id).then((response: any) => {
            for (var j = 0; j < response.data.length; j++) {
                if (response.data[j].userId == userInfo.user_id && response.data[j].prev == 'owner') setIsOwner(true);
                if (response.data[j].userId == userInfo.user_id && response.data[j].prev == 'admin') setIsAdmin(true);
            }
        });
    }, [isAdmin, isOwner]);
    ...
    return (
        <>
            {settings ? (
                <RoomSettings toggleSettings={toggleSettings} roomId={newGroups[searchIndex].id} oldRoomData={oldRoomData} />
            ) : newMembersDash ? (
                <AddMemebers toggleNewMembers={toggleNewMembers} roomId={newGroups[searchIndex].id} />
            ) : (
